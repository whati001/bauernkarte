...
pub async fn find(pool: &PgPool, id: i64) -> sqlx::Result<Option<Product>> {
    sqlx::query_as!(
        Product,
        "select id, name, description, approved, deleted from product where id = $1",
        id
    )
    .fetch_optional(pool)
    .await
...
pub async fn find_live_by_name(pool: &PgPool, name: &str) -> sqlx::Result<Option<Product>> {
    sqlx::query_as!(
        Product,
        "select id, name, description, approved, deleted from product
...
pub async fn insert(pool: &PgPool, name: &str, description: Option<&str>, created_by: i64) -> sqlx::Result<Product> {
    sqlx::query_as!(
        ...
        r#"insert into product (name, description, approved, created_by, modified_by)
           ...
           returning id, name, description, approved, deleted"#,
        name,
        description,
        created_by
    )
...
pub async fn update(
    ...
    sqlx::query_as!(
        Product,
        r#"update product set name = $2, description = $3, modified_by = $4, modified = now()
           where id = $1
           returning id, name, description, approved, deleted"#,
        id,
        name,
...
pub fn snapshot(product: &Product) -> serde_json::Value {
    json!({
        ...
        "approved": product.approved, "deleted": product.deleted,
    })
}
