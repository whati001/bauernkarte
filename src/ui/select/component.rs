...
/// What an empty select shows. The library's own default is the English
/// "Select an option"; the app provides this at the root so both
/// languages read right (see `app::Provide`).
#[derive(Clone, PartialEq)]
pub struct SelectPlaceholder(pub String);
    ...
    rsx! {
        select::Select {
            ...
            select::SelectTrigger {
                ...
                select::SelectValue {
                    placeholder: try_consume_context::<SelectPlaceholder>()
                        .map(|p| p.0)
