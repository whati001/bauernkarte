...
use crate::{
    ...
    error::AppResult,
    handlers::search::{render_search_panel, run_search, SearchQuery, AUSTRIA_LAT, AUSTRIA_LON},
...
pub async fn index(State(state): State<AppState>, OptionalUser(user): OptionalUser) -> AppResult<impl IntoResponse> {
    ...
    let sidebar_html = render_search_panel(&state, None, None, &results).await?;
...
pub async fn store_page(
    ...
    Ok(full_page(&title, user.map(|u| u.name), &signals, sidebar_html).into_response())
}
