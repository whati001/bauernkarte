...
`python3 bootstrap.py ...` also works if `click` is already installed.

Everything is driven by `.env`: `env` writes it (with random secrets),
docker-compose.yml interpolates the same keys, and every other
subcommand reads it. Change a value there and every subcommand follows.

Subcommands:
    env      write `.env` with random secrets
...
# bootstrap.md before changing them on an existing install.
DB_USER={DB_USER}
DB_PWD={DB_PWD}
DB_NAME={DB_NAME}
...
# `cargo sqlx prepare`. Re-run `./bootstrap.py env --force` after
# changing any DB_* value.
...
# a change made later through the app is not overwritten.
...
def load_env():
    ...
    if missing:
        raise click.ClickException(
            ...
            "the current layout. Add them by hand (see .env.example), or back "
            "it up and re-run `./bootstrap.py env --force`."
        )
...
def require_running_db(compose, cfg):
    if not db_is_ready(compose, cfg):
        raise click.ClickException(
            ...
            "— run `./bootstrap.py db` (or `up`) first."
        )
...
def do_backup(out):
    ...
    click.echo(f"wrote {path} — restore with the command in bootstrap.md")
...
def do_cleanup(assume_yes):
    ...
    run(compose + ["down", "--volumes", "--rmi", "local"],
        env=compose_env(cfg))
    click.echo("cleaned up — `./bootstrap.py up` rebuilds from scratch.")
